...
- **Pi（`badlogic/pi-mono`）—— 只借鉴抽象的形状，不追求实现或协议兼容**（[调研笔记](../../research/observability/agent-logging-observability-evidence.md)）。借鉴：最小 port、`SpanStatus` 装不下 stack、span 定义带 parent 约束、错误只记低基数 `error.type`、in-memory 参考实现、两层 containment。**明确不照抄**：其 `sensitive` 标志无 adapter 执行脱敏。
