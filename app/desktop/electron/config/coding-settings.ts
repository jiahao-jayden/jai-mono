...
import { type ConfigMergeCandidate, defineCodingConfig } from "./store";
import type { DesktopMcpServer, ResolvedDesktopProvider } from "./types";
...
class InvalidModelRef extends TaggedError("desktop_provider.invalid_model_ref")<ProviderErrorInit> {}
class ProfileNotFound extends TaggedError("desktop_provider.profile_not_found")<ProviderErrorInit> {}
class ProfileDisabled extends TaggedError("desktop_provider.profile_disabled")<ProviderErrorInit> {}
class ModelNotFound extends TaggedError("desktop_provider.model_not_found")<ProviderErrorInit> {}
class ModelDisabled extends TaggedError("desktop_provider.model_disabled")<ProviderErrorInit> {}
class InvalidConnection extends TaggedError("desktop_provider.invalid_connection")<ProviderErrorInit> {}
class MissingCredentials extends TaggedError("desktop_provider.missing_credentials")<ProviderErrorInit> {}
class UnsupportedReasoningEffort extends TaggedError(
	"desktop_provider.unsupported_reasoning_effort",
)<ProviderErrorInit> {}
class ModelDiscoveryUnsupported extends TaggedError(
	"desktop_provider.model_discovery_unsupported",
)<ProviderErrorInit> {}
class ModelInventoryMissing extends TaggedError("desktop_provider.model_inventory_missing")<ProviderErrorInit> {}
class ModelNotVerified extends TaggedError("desktop_provider.model_not_verified")<ProviderErrorInit> {}
class ModelCapabilityUnsupported extends TaggedError(
	"desktop_provider.model_capability_unsupported",
)<ProviderErrorInit> {}
...
const extensionConfigurationValueSchema = Type.Recursive((This) =>
	Type.Union([
		Type.Null(),
		Type.Boolean(),
		Type.Number(),
...
export const codingAgentSettingsSchema = Type.Object(
	{
		...
		extensions: Type.Optional(Type.Record(Type.String({ minLength: 1 }), extensionConfigurationValueSchema)),
...
export function resolveConfiguredDesktopMcpServers(
	settings: Readonly<CodingAgentSettings>,
): readonly DesktopMcpServer[] {
...
export interface ResolvedDesktopSdkAgentInput {
	readonly model: string;
	readonly provider: CodingProviderOptions;
	readonly maxTurns?: number;
}

export const desktopCodingConfigDefinition = defineCodingConfig({
	schemaVersion: 1,
...
/**
 * Projects a validated Desktop profile selection onto the public SDK's model
 * reference and provider options. The profile ID itself remains Desktop-only.
 */
export function resolveDesktopSdkAgentInput(
	settings: Readonly<CodingAgentSettings>,
	modelRef: string,
	catalog: ModelCatalog | undefined,
	options: ResolveConfiguredProviderOptions,
): ResolvedDesktopSdkAgentInput {
	const resolved = resolveConfiguredProvider(settings, modelRef, catalog, options);
	const separator = modelRef.indexOf("/");
	const profileId = modelRef.slice(0, separator);
	const profile = settings.providers[profileId];
	if (!profile?.adapter) {
	...
	const providerKind =
		profile.adapter === "anthropic"
			...
			: profile.adapter === "openai-responses"
				...
				: "openai-compatible";
	return {
		model: `${providerKind}/${resolved.model.remoteModelId ?? resolved.model.id}`,
		provider: {
			...(profile.apiKey ? { apiKey: profile.apiKey } : {}),
			...(profile.baseURL ? { baseUrl: profile.baseURL } : {}),
			...(profile.headers ? { headers: profile.headers } : {}),
			authentication: profile.auth ?? (profile.adapter === "anthropic" ? "x-api-key" : "bearer"),
		},
		...(settings.agent?.maxIterations ? { maxTurns: settings.agent.maxIterations } : {}),
	};
}

export function resolveConfiguredAgentRuntime(
	settings: Readonly<CodingAgentSettings>,
	resolved: ResolvedDesktopProvider,
