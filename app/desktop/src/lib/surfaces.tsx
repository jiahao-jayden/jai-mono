...
export const fieldInteractive =
	"bg-foreground/[0.04] transition-colors hover:bg-foreground/[0.07] dark:bg-foreground/[0.06]";
...
export const ghostButton =
	"flex items-center justify-center rounded-full text-foreground/45 outline-none transition-[background-color,color,scale] duration-150 hover:bg-hover hover:text-foreground/90 active:scale-[0.96] focus-visible:ring-1 focus-visible:ring-foreground/20 motion-reduce:transition-none";
...
export function SwapLabel({
	...
	useLayoutEffect(() => {
		const target = active === 0 ? firstLayerRef.current : secondLayerRef.current;
		if (!target) return undefined;
		const measure = () => {
			const next = Math.ceil(target.getBoundingClientRect().width);
