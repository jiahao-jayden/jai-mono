...
import { createDesktopConnectorConfigStore } from "../electron/config/connector-store";
...
describe("Desktop Connector config store", () => {
	test("persists Connector config and raw credentials in user settings", async () => {
		...
		roots.push(homeDir);
		const store = createDesktopConnectorConfigStore({ homeDir, workspaceTrusted: true });
	...
	test("returns a conflict when a stale settings revision is used", async () => {
		...
		const initial = await store.load();
		if (initial.isErr()) return;
		const first = await store.save({ policy: { default: "allow" } }, { expectedRevision: initial.value.revision });
