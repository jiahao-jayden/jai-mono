...
import { discoverDesktopAgentPluginDirectories } from "../electron/agent/plugin-directories";
...
describe("桌面端 Agent Plugin 默认目录发现", () => {
	test("按项目与用户优先级发现每个根目录的直接子插件", async () => {
		...
		const directories = await discoverDesktopAgentPluginDirectories({
			homeDirectory: home,
...
async function tempPath(name: string): Promise<string> {
	const root = await mkdtemp(path.join(tmpdir(), `jai-desktop-plugin-${name}-`));
	roots.push(root);
	return root;
}
