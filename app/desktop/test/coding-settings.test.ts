...
import { CodingConfigStore } from "../electron/config/store";
import {
...
describe("Desktop provider configuration", () => {
	test("从 Settings 解析 MCP server，并补齐运行时默认字段", () => {
		...
		expect(resolveConfiguredDesktopMcpServers(settings)).toEqual([
			{
				name: "stdio",
				type: "stdio",
				command: "npx",
	...
	test("Connector 配置只读取 user-global，项目配置不能扩大能力", async () => {
		...
		await writeSettings(join(fixture.homeDir, ".jai", "settings.json"), {
			connector: {
				policy: { default: "allow", actions: { "context7.get_documentation_context": "deny" } },
		...
		const store = new CodingConfigStore(desktopCodingConfigDefinition, {
			...fixture,
			workspaceTrusted: true,
		});
		const snapshot = await store.load();
		expect(snapshot.settings.connector?.policy?.default).toBe("allow");
	...
	test("keeps the current Connector config schema at v1", () => {
		expect(desktopCodingConfigDefinition.schemaVersion).toBe(1);
	});
	...
	test("rejects removed Connector fields without a compatibility layer", async () => {
		...
		await expect(store.load()).rejects.toMatchObject({ _tag: "desktop_config.validation_failed" });
		store.close();
	});
...
async function writeSettings(path: string, settings: Record<string, unknown>): Promise<void> {
	...
	await writeFile(
		path,
		`${JSON.stringify({
			$schema: desktopCodingConfigDefinition.schemaUrl,
			schemaVersion: desktopCodingConfigDefinition.schemaVersion,
			...settings,
		})}\n`,
	);
}
