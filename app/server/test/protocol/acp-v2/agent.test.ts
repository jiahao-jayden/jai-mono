...
describe("ACP v2 Agent adapter", () => {
	...
	test("projects disposable live chunks while durable entries remain the replay frontier", async () => {
		...
		driver.emit({ type: "tool_activity", toolCallId: "tool-1", activityTitle: "Grep" });
		driver.emit({
			type: "tool_details",
			toolCallId: "tool-1",
			toolName: "web_search",
			...
			},
			{
				...
				params: {
					...
					update: {
						...
						status: "in_progress",
						rawInput: { path: "package.json" },
						_meta: { jai: { operationId: "operation-1", toolName: "Read" } },
					},
				},
			},
			{
			...
			},
			{
				...
				params: {
					...
					update: {
						sessionUpdate: "tool_call_update",
						toolCallId: "tool-1",
						_meta: { jai: { operationId: "operation-1", activityTitle: "Grep" } },
					},
				},
			},
			{
		...
		expect(agent.drain()).toEqual([
			{
				...
				params: {
					...
					update: {
						...
						_meta: { jai: { operationId: "operation-1", toolName: "Bash" } },
					},
				},
			},
			{
		...
		expect(agent.drain()).toEqual([
			{
				...
				params: {
					...
					update: {
						...
						_meta: { jai: { operationId: "operation-1", toolName: "Bash", completedAt: 1_787_745_600_000 } },
					},
				},
			},
			{
		...
		expect(live.drain()).toEqual([
			{
				...
				params: {
					...
					update: {
						...
						status: "pending",
						rawInput: {},
						_meta: { jai: { operationId: "operation-1", toolName: "Write", startedAt: 1_788_393_600_000 } },
					},
				},
			},
		]);
		...
		expect(live.drain()).toEqual([
			{
				...
				params: {
					...
					update: {
						...
						status: "in_progress",
						rawInput: { path: "index.ts", content: "export {};" },
						_meta: { jai: { operationId: "operation-1", toolName: "Write" } },
					},
				},
			},
		]);
		...
		const expectedResult = {
			...
			params: {
				...
				update: {
					...
					_meta: { jai: { operationId: "operation-1", toolName: "Write", completedAt: 1_787_745_600_000 } },
				},
			},
