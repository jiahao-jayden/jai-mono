...
export class RuntimeAgentSettingsMissing extends TaggedError(
  "runtime_config.agent_settings_missing",
)<{
  readonly message: string;
}> {}

export class RuntimeAgentSettingsCorrupted extends TaggedError(
  "runtime_config.agent_settings_corrupted",
)<{
  readonly message: string;
  readonly cause?: unknown;
}> {}

export class RuntimeAgentSettingsInvalid extends TaggedError(
  "runtime_config.agent_settings_invalid",
)<{
  readonly message: string;
}> {}

export class RuntimeAgentSettingsWriteConflict extends TaggedError(
...
export class RuntimeAgentSettingsModelFetchFailed extends TaggedError(
  "runtime_config.model_fetch_failed",
)<{
  readonly profileId: string;
  readonly message: string;
  readonly cause?: unknown;
...
export type RuntimeAgentSettingsReadError =
...
const sensitiveHeaderPattern =
  /^(authorization|proxy-authorization|x-api-key)$/i;
...
export class SqliteRuntimeAgentSettings {
  ...
  bootstrap(
    ...
    void,
    RuntimeAgentSettingsInvalid | RuntimeAgentSettingsCorrupted
  > {
  ...
  > {
    ...
    return Result.ok(
      projectSnapshot(current.value.settings, current.value.revision),
    );
  }
        ...
        if (parsed.revision !== null) {
          return Result.err(
            new RuntimeAgentSettingsWriteConflict({
              message:
                "Runtime Agent settings were not configured at the requested revision",
  ...
  async fetchModels(
    ...
    ResultType<
      ...
      RuntimeAgentSettingsModelFetchError
    >
  > {
   ...
   */
  readConnectorSettings(): ResultType<
  ...
  > {
    ...
    const credentials = {
      ...
      ...(options.oauthIntentId === undefined
        ? {}
        : { oauthIntentId: options.oauthIntentId }),
    };
    return this.persist(
      {
        ...current.value.settings,
  ...
  > {
    if (!connectorId.trim())
      return Result.err(
        new RuntimeAgentSettingsInvalid({ message: "Connector id is invalid" }),
      );
    const current = this.current();
    if (current.isErr()) return Result.err(current.error);
    const connectors = current.value.settings.connector?.connectors ?? {};
    ...
    const {
      credentials: _previousCredentials,
  ...
  private insertInitial(
    ...
    try {
      ...
      return Result.ok(
        projectSnapshot(initial.value, revisionFor(initial.value)),
      );
    } catch (cause) {
      return Result.err(
        new RuntimeAgentSettingsCorrupted({
  ...
  private persist(
    ...
    try {
      ...
      const row = this.database
        .prepare(
          "SELECT settings_json FROM runtime_agent_settings WHERE key = 'default'",
        )
      ...
      if (!row || revisionForParsed(row.settings_json) !== expectedRevision) {
        ...
        return Result.err(
          new RuntimeAgentSettingsWriteConflict({
            message:
              "Runtime Agent settings changed before they could be saved",
            expectedRevision,
      ...
      this.database
        .prepare(
          "UPDATE runtime_agent_settings SET settings_json = ?, updated_at = ? WHERE key = 'default'",
        )
        .run(encoded, now);
      this.database.exec("COMMIT");
      return Result.ok(
        projectSnapshot(validated.value, revisionFor(validated.value)),
      );
    } catch (cause) {
      try {
        this.database.exec("ROLLBACK");
...
export function parseRuntimeAgentSettingsInput(
  value: unknown,
): RuntimeAgentSettingsInput | undefined {
  if (
    !isRecord(value) ||
    !hasOnly(value, [
      "revision",
  ...
  if (
    ...
    !isRuntimeConnectorSettings(value.connector)
  )
    return undefined;
...
export function resolveRuntimeAgentOptions(
  ...
  const remoteModelId =
    separator > 0 ? settings.model.slice(separator + 1) : "";
  const profile = profileId ? settings.providers[profileId] : undefined;
  const execution = resolveExecutionOptions(
    settings,
    profile ? sdkProviderKind(profile.adapter) : profileId,
  );
  ...
  const selected = Object.values(profile.models).find(
    (model) => (model.remoteModelId ?? model.id) === remoteModelId,
  );
...
function resolveExecutionOptions(
  ...
  Pick<ResolvedRuntimeAgentOptions, "instructions" | "providerOptions">,
  RuntimeAgentSettingsInvalid
> {
  const instructions = settings.language
...
function parseProfileInput(
  value: unknown,
): RuntimeProviderProfileInput | undefined {
  if (
    !isRecord(value) ||
    !hasOnly(value, [
      "id",
...
function parseProviderModel(value: unknown): RuntimeProviderModel | undefined {
  if (!isRecord(value) || !hasOnly(value, ["id", "remoteModelId", "enabled"]))
    return undefined;
  if (
    typeof value.id !== "string" ||
    (value.remoteModelId !== undefined &&
      typeof value.remoteModelId !== "string") ||
    typeof value.enabled !== "boolean"
  ) {
    return undefined;
  }
...
function settingsFromInput(
  ...
  for (const inputProfile of input.providers) {
    ...
    if (
      inputProfile.previousId !== undefined &&
      !profileIdPattern.test(inputProfile.previousId)
    ) {
      return Result.err(
        new RuntimeAgentSettingsInvalid({
    ...
    const previous =
      ...
      (inputProfile.previousId === undefined
        ? undefined
        : current.providers[inputProfile.previousId]);
...
function normalizeProfile(
  ...
  const headers =
    input.headers === undefined
      ...
      : Object.keys(input.headers).length > 0
        ...
        : undefined;
  if (headers instanceof RuntimeAgentSettingsInvalid)
    return Result.err(headers);
  ...
  const apiKey = input.clearApiKey
    ? undefined
    : input.apiKey?.trim() ||
      (connectionChanged ? undefined : previous?.apiKey);
  ...
  for (const model of input.models) {
    ...
    if (
      ...
      (remoteModelId !== undefined && !modelIdPattern.test(remoteModelId))
    ) {
      return Result.err(
        new RuntimeAgentSettingsInvalid({
  ...
  return Result.ok({
    ...
    models,
    ...(previous?.modelInventory === undefined
      ? {}
...
function validateSettings(
  ...
  if (
    typeof value.model !== "string" ||
    (value.model !== "" && !validModelReference(value.model))
  ) {
    return Result.err(
      new RuntimeAgentSettingsInvalid({
  ...
  for (const [profileId, profile] of Object.entries(value.providers)) {
    ...
    providers[profileId] = {
      ...normalized.value,
      ...(parsed.modelInventory === undefined
        ? {}
  ...
  const settings: RuntimeAgentSettings = {
    ...
    ...(reasoningEffort === undefined ? {} : { reasoningEffort }),
    providers,
    extensions: structuredClone(value.extensions) as Readonly<
      Record<string, JsonObject>
    >,
    ...(value.connector === undefined
      ? {}
      : {
          connector: structuredClone(
            value.connector,
          ) as RuntimeConnectorSettings,
        }),
  };
  const separator = settings.model.indexOf("/");
  const profileId = separator > 0 ? settings.model.slice(0, separator) : "";
...
function parseStoredProfile(
  value: unknown,
): RuntimeProviderProfile | undefined {
  if (
    !isRecord(value) ||
    !hasOnly(value, [
      "name",
  ...
  if (
    ...
    (value.modelInventory !== undefined &&
      !isProviderModelInventory(value.modelInventory))
  ) {
    return undefined;
  }
  const models: Record<string, RuntimeProviderModel> = {};
  ...
  return {
    ...
    profiles: Object.entries(settings.providers)
      .map(([id, profile]) => ({
        ...
        ...(profile.apiKey
          ? { credentialMask: maskCredential(profile.apiKey) }
          : {}),
        enabled: profile.enabled,
        ...(profile.modelInventory === undefined
          ? {}
          : { modelsFetchedAt: profile.modelInventory.fetchedAt }),
...
): readonly RuntimeProviderModel[] {
  const configuredByRemoteId = new Map(
    Object.values(profile.models).map((model) => [
      model.remoteModelId ?? model.id,
      model,
    ]),
  );
  const remoteModelIds = profile.modelInventory?.modelIds ?? [
    ...configuredByRemoteId.keys(),
  ];
  return remoteModelIds
    .map((remoteModelId) => {
      ...
      return {
        ...
        ...(configured?.remoteModelId || configured?.id !== remoteModelId
          ? { remoteModelId }
          : {}),
    ...
    .toSorted((left, right) =>
      (left.remoteModelId ?? left.id).localeCompare(
        right.remoteModelId ?? right.id,
      ),
    );
}
  ...
  const provider: Provider =
    profile.adapter === "anthropic"
      ? new AnthropicProvider({
          ...shared,
          authentication:
            profile.authentication === "none" ? "none" : "x-api-key",
        })
      : profile.adapter === "openai-responses"
        ? new OpenAIResponsesProvider({
            ...shared,
            authentication:
              profile.authentication === "none" ? "none" : "bearer",
          })
        : new OpenAIProvider({
            ...shared,
            authentication:
...
function isProviderModelInventory(
  value: unknown,
): value is RuntimeProviderModelInventory {
  return (
    isRecord(value) &&
    Array.isArray(value.modelIds) &&
    value.modelIds.every(
...
function projectConnector(
  value: RuntimeConnectorSettings | undefined,
...
function validConnectorOAuthToken(
  value: RuntimeConnectorOAuthTokenInput,
): boolean {
  return (
...
function isRuntimeConnectorPolicy(
  value: unknown,
): value is NonNullable<RuntimeConnectorSettings["policy"]> {
  return (
    isRecord(value) &&
    ...
    (value.actions === undefined ||
      (isRecord(value.actions) &&
        Object.values(value.actions).every(isConnectorPermission)))
  );
}

function isRuntimeConnectorSettings(
  value: unknown,
): value is RuntimeConnectorSettings {
  if (!isRecord(value) || !hasOnly(value, ["policy", "connectors"]))
    return false;
  ...
  if (value.connectors !== undefined) {
    ...
    for (const [id, connector] of Object.entries(value.connectors)) {
      if (
        !id.trim() ||
        !isRecord(connector) ||
        !hasOnly(connector, ["enabled", "credentials"])
      )
        return false;
      if (
        ...
        return false;
      if (
        connector.credentials !== undefined &&
...
function isConnectorPermission(
  value: unknown,
): value is RuntimeConnectorPermission {
...
function validateBaseURL(
  profileId: string,
  value: string,
): ResultType<void, RuntimeAgentSettingsInvalid> {
  try {
...
function normalizeHeaders(
  ...
  for (const [name, headerValue] of Object.entries(value)) {
    if (
      !name.trim() ||
      !headerValue.trim() ||
      sensitiveHeaderPattern.test(name)
    ) {
...
function isAuthentication(
  value: unknown,
): value is RuntimeProviderAuthentication {
...
function isJsonObjectRecord(
  value: unknown,
): value is Readonly<Record<string, JsonObject>> {
