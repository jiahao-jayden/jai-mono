...
export class RuntimeHostSessionNotFound extends TaggedError(
  "runtime_host.session_not_found",
)<{
  readonly sessionId: string;
  readonly message: string;
}> {}

export class RuntimeHostSessionAlreadyExists extends TaggedError(
  "runtime_host.session_already_exists",
)<{
  readonly sessionId: string;
  readonly message: string;
}> {}

export class RuntimeHostPromptRejected extends TaggedError(
  "runtime_host.prompt_rejected",
)<{
  readonly sessionId: string;
  readonly message: string;
...
export class RuntimeHostRecoveryCorrupted extends TaggedError(
  "runtime_host.recovery_corrupted",
)<{
  readonly sessionId: string;
  readonly message: string;
  readonly cause?: unknown;
...
export class RuntimeHostIndeterminateTool extends TaggedError(
  "runtime_host.indeterminate_tool",
)<{
  readonly sessionId: string;
  readonly operationId: string;
  readonly message: string;
...
export class RuntimeHostSessionControllerHeld extends TaggedError(
  "runtime_host.session_controller_held",
)<{
  readonly sessionId: string;
  readonly message: string;
}> {}
...
export class RuntimeHostSessionBusy extends TaggedError(
  "runtime_host.session_busy",
)<{
  readonly sessionId: string;
  readonly operationId: string;
  readonly message: string;
...
export class RuntimeHostApprovalNotFound extends TaggedError(
  "runtime_host.approval_not_found",
)<{
  readonly sessionId: string;
...
export class RuntimeHostConfigurationRejected extends TaggedError(
  "runtime_host.configuration_rejected",
)<{
  readonly sessionId: string;
  readonly message: string;
  readonly cause?: unknown;
...
class RuntimeHostApprovalCancelled extends TaggedError(
  "runtime_host.approval_cancelled",
)<{
  readonly sessionId: string;
  readonly requestId: string;
  readonly message: string;
...
export type RuntimeHostRecoveryError =
  | RuntimeHostPromptRejected
  | RuntimeHostRecoveryCorrupted;
...
export type RuntimeHostPromptError =
  | RuntimeHostPromptRejected
  | RuntimeHostSessionBusy
  | RuntimeHostIndeterminateTool;
export type RuntimeHostSnapshotError =
  | RuntimeHostPromptRejected
  | RuntimeHostRecoveryCorrupted;
export type RuntimeHostApprovalError =
  | RuntimeHostPromptRejected
  | RuntimeHostApprovalNotFound;
export type RuntimeHostConfigurationError =
  | RuntimeHostPromptRejected
  | RuntimeHostConfigurationRejected;
...
export interface RuntimeSession {
  ...
  configuration(): Promise<
    Result<RuntimeSessionConfigurationSnapshot, RuntimeHostConfigurationError>
  >;
  setConfiguration(
...
export interface RuntimeHost {
  listSessions(): Promise<
    Result<readonly ProductSessionInfo[], RuntimeHostPromptRejected>
  >;
  openSession(
...
class DefaultRuntimeHost implements RuntimeHost {
  ...
  readonly #ephemeralPersistences = new Map<
    string,
    ProductSessionPersistence
  >();
  ...
  async openSession(
    ...
    if (input.kind === "resume") {
      ...
      const finalized = await this.finalizeInferredTerminals(
        found.value,
        recovery.value,
      );
      if (finalized.isErr()) {
    ...
    if (
      this.#pendingSessionIds.has(id) ||
      this.#ephemeralPersistences.has(id)
    ) {
      return Result.err(
  ...
  private async openNewSession(
    ...
    const initialConfiguration =
      await this.#configurationPolicy.initialConfiguration();
    ...
    const ephemeralPersistence = input.ephemeral
      ? this.options.createEphemeralPersistence?.()
      : undefined;
    ...
    if (created.isErr())
      return Result.err(this.projectOpenError(created.error));
    ...
    if (held) return Result.err(held);
    if (!ephemeralPersistence)
      return Result.ok(this.runtimeSession(durable.value, input.controllerId));
  ...
  > {
    ...
    for (const verdict of inferred) {
      ...
      if (appended.isErr())
        return Result.err(this.projectOpenError(appended.error));
    }

    const reloaded = await this.options.persistence.load(state.id);
    if (reloaded.isErr())
      return Result.err(this.projectOpenError(reloaded.error));
  ...
  private runtimeSession(
    ...
    releaseController: () => void = () =>
      this.releaseController(state.id, controllerId),
    ...
    session = new DefaultRuntimeSession(
      ...
      discardWhenClosed,
      () => {
        if (this.#liveSessions.get(state.id) === session)
          this.#liveSessions.delete(state.id);
      },
    );
  ...
  private releaseController(
    ...
    if (
      ...
      this.#controllers.get(sessionId) === controllerId
    ) {
...
class DefaultRuntimeSession implements RuntimeSession {
  ...
  async prompt(
    ...
    const admitted = await this.enqueue(async () => {
      ...
      if (this.operationDriver)
        this.#active = createActiveOperation(operationId);
  ...
  resume(
    verdicts: readonly OperationRecoveryVerdict[],
  ): Result<void, RuntimeHostRecoveryCorrupted> {
    ...
    this.#active = createActiveOperation(
      verdict.operationId,
      queuedInputsFor(verdict),
    );
  ...
  async snapshot(): Promise<
    Result<RuntimeSessionSnapshot, RuntimeHostSnapshotError>
  > {
    if (this.#closed) return Result.err(this.closed());
    const loaded = await this.persistence.load(this.id);
    if (loaded.isErr()) return Result.err(this.reject(loaded.error));
  ...
  > {
    ...
    return this.enqueue(async () => {
      const loaded = await this.persistence.load(this.id);
      if (loaded.isErr()) return Result.err(this.reject(loaded.error));
      const next = await this.applyConfigurationChange(
        loaded.value.runtimeConfiguration,
        input,
      );
  ...
  async cancel(): Promise<
    Result<RuntimeCancelOutcome, RuntimeHostCancelError>
  > {
    if (this.#closed) return Result.err(this.closed());
    if (this.#suspended) {
      return Result.err(
    ...
    if (this.#indeterminate) return Result.err(this.#indeterminate);
    if (this.operationDriver && this.#active)
      return this.cancelLiveOperation(this.#active);
    const cancelled = await this.enqueue(async () => {
      const loaded = await this.persistence.load(this.id);
      if (loaded.isErr()) return Result.err(this.reject(loaded.error));
      const recovered = recoverDurableState(loaded.value);
      if (recovered.isErr()) return recovered;
      const active = recovered.value.find(
        (verdict) => verdict.status !== "terminal",
      );
  ...
  async close(): Promise<void> {
    ...
    if (this.discardWhenClosed && this.#active) {
      ...
      if (this.#active.resource)
        void this.#active.resource.abort().catch(() => {});
    }
  ...
  private async applyConfigurationChange(
    ...
    Result<RuntimeSessionConfiguration, RuntimeHostConfigurationRejected>
  > {
    ...
    if (models.isErr())
      return Result.err(this.rejectConfiguration(models.error));
  ...
  private rejectConfiguration(
    error: RuntimeSessionConfigurationInvalid,
  ): RuntimeHostConfigurationRejected {
  ...
  private async runOperation(
    ...
    Result<
      ...
      RuntimeOperationExecutionFailed | RuntimeHostIndeterminateTool
    >
  > {
    ...
    const opened = await driver.openOperation({
      ...
      requestApproval: (request, signal) =>
        this.requestApproval(request, signal),
    });
    if (opened.isErr()) {
      return this.completeOperation(
        active,
    ...
    active.stopObserving = opened.value.subscribe?.((event) => {
      if (event.type === "usage_settled") {
        ...
        if (this.#usageCost !== previousCost)
          this.publish({ type: "usage_changed", cost: this.#usageCost });
        return;
      }
      this.publish({
    ...
    try {
      if (active.abortRequested) {
        ...
        if (aborted.isErr())
          return this.completeOperation(active, Result.err(aborted.error));
      }
      return await this.completeOperation(
        active,
        await active.resource.awaitOutcome(),
      );
    } catch (error) {
      return this.completeOperation(
        active,
  ...
  > {
    return this.enqueue(async () => {
      ...
      const verdict = recovered.value.find(
        (candidate) => candidate.operationId === active.operationId,
      );
      if (verdict?.status === "indeterminate_tool") {
      ...
      const terminalOutcome =
        inferredTerminalOutcome ?? (outcome.isOk() ? outcome.value : "failed");
      ...
      return inferredTerminalOutcome
        ? Result.ok(inferredTerminalOutcome)
        : outcome;
    });
  }
  ...
  private async cancelLiveOperation(
    ...
    if (outcome.isErr()) {
      if (outcome.error instanceof RuntimeHostIndeterminateTool)
  ...
  private foregroundState(
    ...
    if (
      this.#indeterminate ||
      recovery.some((verdict) => verdict.status === "indeterminate_tool")
    ) {
    ...
    if (
      this.#active ||
      recovery.some((verdict) => verdict.status !== "terminal")
    ) {
      return { state: "running" };
    }
    const terminal = [...state.operationRecords]
      .reverse()
      .find((record) => record.type === "operation_finished");
    return terminal
      ? { state: "idle", stopReason: stopReasonFor(terminal.outcome) }
      : { state: "idle" };
  }
  ...
  private suspend(
    active: ActiveOperation,
    error: RuntimeOperationExecutionFailed,
  ): void {
  ...
  private requestApproval: RuntimeApprovalHandler = (request, signal) => {
    if (this.#closed)
      return Promise.reject(
        this.approvalCancelled(request.requestId, "Runtime Session was closed"),
      );
    if (request.sessionId !== this.id) {
      return Promise.reject(
        this.approvalCancelled(
          request.requestId,
  ...
  private cancelPendingApprovals(message: string): void {
    for (const requestId of [...this.#pendingApprovals.keys()]) {
      this.#takeApproval(requestId)?.reject(
        this.approvalCancelled(requestId, message),
      );
    }
  }

  private approvalCancelled(
    requestId: string,
    message: string,
  ): RuntimeHostApprovalCancelled {
...
function stopReasonFor(
  outcome: RuntimeOperationOutcome | "blocked",
): RuntimeStopReason {
  switch (outcome) {
    case "completed":
      return "end_turn";
...
function usageCost(
  ...
  return records.reduce(
    (total, record) =>
      record.type === "usage_settled"
        ? total + finiteCost(record.usage.cost.total)
        : total,
    0,
  );
}
...
function queuedInputsFor(
  verdict: OperationRecoveryVerdict,
): readonly RuntimeQueuedInput[] {
...
function hasPendingInputs(
  verdict: OperationRecoveryVerdict | undefined,
): boolean {
  return (
    (verdict?.status === "ready" ||
      verdict?.status === "provider_interrupted") &&
...
function terminalOutcomeForAssistant(
  message: AssistantMessage,
): OperationFinished["outcome"] | undefined {
