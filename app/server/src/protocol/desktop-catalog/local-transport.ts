...
import type { DesktopCatalogControl } from "./control";

export class DesktopCatalogControlListenFailed extends TaggedError("desktop_catalog_control.listen_failed")<{
 ...
 */
export async function openLocalDesktopCatalogControlServer(input: {
	readonly endpoint: string;
	readonly control: DesktopCatalogControl;
}): Promise<ResultType<LocalDesktopCatalogControlServer, DesktopCatalogControlListenFailed>> {
	...
	try {
		...
		server.close();
		return Result.err(
			new DesktopCatalogControlListenFailed({
				message: `Could not listen for Desktop Catalog control at "${input.endpoint}"`,
...
function openConnection(socket: Socket, control: DesktopCatalogControl): void {
	let buffered = "";
...
async function handleLine(socket: Socket, control: DesktopCatalogControl, line: string): Promise<void> {
	let message: unknown;
	try {
...
function write(
	...
	try {
		...
		// A dead client has no effect on durable Catalog facts.
	}
}
