...
- 单文件可到约 800 行；600 行前评估是否混入多个领域或角色。只有职责混杂、接口不清或拆分能显著改善定位时，才按领域文件夹拆分；不要把几行函数拆到同级文件来降低行数。
...
- 不保留向后兼容：过时的直接删，不加兼容层、migration 或 fallback。
- 选满足当前需求的最简单实现，不做预防性抽象和多余配置层；先跑通最小端到端版本再往上加。
- 先查已有依赖和成熟库，再考虑新增包或自己写；参考成熟产品的已验证模式，不从零发明。
