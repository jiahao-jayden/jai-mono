...
对于跨模块迁移、兼容性收敛或架构重组，拆分前必须先完成“现状迁移清单”。每一条行为都要写清：当前代码位置、触发条件、当前实际输入/输出或副作用、目标 owner 与调用链、调用方如何接入、Adapter 要删除的旧逻辑、行为保持还是改变及原因、对应测试或 fixture。只列文件名、模块名或“定义 contract/迁移逻辑”不算完成。若清单、端到端案例或公共冲突/未知规则仍未确定，继续调查或回到 `jn-intent`，不得输出 `jn:awaiting-confirmation` 的实施拆分。
...
每项迁移任务的验收必须能反查到上述清单：指出迁移前后的具体行为、旧分支删除位置、调用链接线和保持/改变行为的回归用例。架构决策不能用“由执行者定义”留空；如果必须由实施者现场设计，任务仍属于调查/方案阶段，不得作为执行授权发布。
