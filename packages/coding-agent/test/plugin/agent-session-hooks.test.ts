...
describe("AgentSession plugin integration", () => {
  ...
  test("loads plugins from ~/.jai/plugins and cwd/.jai/plugins", async () => {
    ...
    await writeFile(
      join(userPluginDir, "plugin.json"),
      JSON.stringify({ name: "user-demo", version: "0.1.0" }),
    );
    await writeFile(
      join(userPluginDir, "commands", "hello.md"),
  ...
  test("preToolCall hook sees events and can skip execution", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "interceptor", version: "0.1.0" }),
    );
    await writeFile(
  ...
  test("preCompact hook receives event and can skip", async () => {
    ...
    await writeFile(
      join(pluginDir, "plugin.json"),
      JSON.stringify({ name: "no-compact", version: "0.1.0" }),
    );
    await writeFile(
      join(pluginDir, "index.ts"),
      `
