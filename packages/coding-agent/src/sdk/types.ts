...
/** A completed, durable workspace mutation associated with one tool result. */
...
export type CodingAgentMessage =
  | {
      ...
      readonly content:
        | string
        | readonly (CodingTextContent | CodingImageContent)[];
...
export type CodingAssistantMessage = Extract<
  CodingAgentMessage,
  { readonly role: "assistant" }
>;
export type CodingToolResult = Extract<
  CodingAgentMessage,
  { readonly role: "toolResult" }
>;
...
/** A Host-reserved Session entry identity for an input delivered at a safe checkpoint. */
export interface CodingQueuedInput {
  readonly text: string;
...
export type CodingAgentEvent =
  ...
  | {
      ...
      readonly assistantEvent:
        ...
        | {
            readonly type: "done";
            readonly reason: CodingStopReason;
            readonly message: CodingAssistantMessage;
          }
        | {
            readonly type: "error";
            readonly reason: "error" | "aborted";
            readonly error: CodingAssistantMessage;
          };
    }
  | {
...
export interface CodingAgent<TAppState extends JsonObject = JsonObject> {
  ...
  ): Promise<Result<CodingRunResult<TAppState>, CodingSdkError>>;
  /** Advances an already-admitted durable input without adding a second user message. */
  advance(
    inputs?: readonly CodingQueuedInput[],
  ): Promise<Result<CodingRunResult<TAppState>, CodingSdkError>>;
  steer(
    input: string | CodingQueuedInput,
  ): Promise<Result<void, CodingSdkError>>;
  followUp(
    input: string | CodingQueuedInput,
  ): Promise<Result<void, CodingSdkError>>;
