...
export async function createCodingAgent<TAppState extends JsonObject = JsonObject>(
	...
	try {
		...
		const fileCapabilities = resolveFileCapabilities(input, session, ephemeralDirectory);
...
class PublicCodingAgent<TAppState extends JsonObject> implements CodingAgent<TAppState> {
		...
		const run = this.#tail.then(async () => {
			...
			try {
				const messages = options?.attachments?.length
					...
					: await this.#internal.invoke(prompt);
				return {
					result: {
						sessionId: this.#sessionId,
	...
	advance(inputs: readonly CodingQueuedInput[] = []): Promise<ResultType<CodingRunResult<TAppState>, CodingSdkError>> {
		let admittedEpoch: number | undefined;
		const run = this.#tail.then(async () => {
			if (this.#closed) throw agentClosedFailure();
			const admissionEpoch = ++this.#admissionEpoch;
		...
		return run.then(
			(value) => Result.ok<CodingRunResult<TAppState>, CodingSdkError>(value.result),
			(error) => Result.err<CodingRunResult<TAppState>, CodingSdkError>(projectError(error, "model")),
		);
	}

	steer(input: string | CodingQueuedInput): Promise<ResultType<void, CodingSdkError>> {
		return this.#admit(input, (message, entryId) => this.#internal.steer(message, entryId));
	}

	followUp(input: string | CodingQueuedInput): Promise<ResultType<void, CodingSdkError>> {
		return this.#admit(input, (message, entryId) => this.#internal.followUp(message, entryId));
	}
